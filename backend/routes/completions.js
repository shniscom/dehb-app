...
router.post('/', (req, res) => {
  ...
  const result = db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?)
  `).run(task_id, req.user.id, quality||'ok', was_late?1:0, pts, photo_url||null, JSON.stringify(subtasks_done||[]), today);
  res.status(201).json({ id: result.lastInsertRowid, status:'pending', pts_preview:pts });
});
...
router.get('/pending', parentOnly, (req, res) => {
  ...
  res.json(pending.map(p=>({...p, subtasks:JSON.parse(p.subtasks||'[]'), subtasks_done:JSON.parse(p.subtasks_done||'[]')})));
});
...
router.patch('/:id/approve', parentOnly, (req, res) => {
  ...
  let pts = calcPoints(comp, finalQuality);
  ...
  db.transaction(()=>{
    const subsSql = subtasks_override ? ', subtasks_done=?' : '';
    const args = [finalQuality, pts, parent_note||null, now, bonus, behavior_note||null,
      ...(subtasks_override?[JSON.stringify(subtasks_override)]:[]), comp.id];
    db.prepare(`UPDATE completions SET status='approved',quality=?,pts_awarded=?,parent_note=?,approved_at=?,behavior_bonus=?,behavior_note=?${subsSql} WHERE id=?`).run(...args);
    db.prepare(`INSERT INTO point_ledger (child_id,delta,reason,source_type,source_id) VALUES (?,?,?,'completion',?)`).run(comp.child_id, pts, `Görev onayı: ${comp.id}`, comp.id);
...
router.delete('/reactivate/:taskId', parentOnly, (req, res) => {
  ...
  db.transaction(()=>{
    if (comp.status==='approved' && comp.pts_awarded>0) {
      db.prepare(`INSERT INTO point_ledger (child_id,delta,reason,source_type,source_id) VALUES (?,?,'Görev reaktive edildi','manual',?)`).run(comp.child_id, -comp.pts_awarded, comp.id);
      const total=calcTotal(db,comp.child_id);
      db.prepare('UPDATE users SET total_points=? WHERE id=?').run(total, comp.child_id);
    }
